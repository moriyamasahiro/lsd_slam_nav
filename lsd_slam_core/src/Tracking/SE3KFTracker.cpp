...
SE3KFTracker::SE3KFTracker(int w, int h, Eigen::Matrix3f K)
{
	width = w;
	height = h;
...
SE3 SE3KFTracker::trackFrameSE3KF(
		...
		int startLevel, int finalLevel)
{
	...
	SE3ResidualStruct finalResidual;
	...
	for(int lvl=startLevel;lvl >= finalLevel;lvl--)
	{
		...
		callOptimized(calcSE3Buffers, (reference, frame, referenceToFrame, lvl));
		...
		SE3ResidualStruct lastErr = callOptimized(calcSE3WeightsAndResidual,(referenceToFrame));
		if(plotSim3TrackingIterationInfo) callOptimized(calcSE3Buffers,(reference, frame, referenceToFrame, lvl, true));
		...
		for(int iteration=0; iteration < settings.maxItsPerLvl[lvl]; iteration++)
		{
			...
			callOptimized(calcSE3LGS,(ls7_kf));
			...
			while(true)
			{
				...
				Vector6 b = - ls7_kf.b / ls7_kf.num_constraints;
				Matrix6x6 A = ls7_kf.A / ls7_kf.num_constraints;
				...
				//SE3 new_referenceToFrame = referenceToFrame * SE3::exp((inc));


				// re-evaluate residual
				callOptimized(calcSE3Buffers,(reference, frame, new_referenceToFrame, lvl));
				if(buf_warped_size < 0.5 * MIN_GOODPERALL_PIXEL_ABSMIN * (width>>lvl)*(height>>lvl) || buf_warped_size < 10)
				{
					diverged = true;
					return SE3();
				...
				SE3ResidualStruct error = callOptimized(calcSE3WeightsAndResidual,(new_referenceToFrame));
				if(plotSim3TrackingIterationInfo) callOptimized(calcSE3Buffers,(reference, frame, new_referenceToFrame, lvl, true));
				numCalcResidualCalls[lvl]++;
	...
	if (!warp_update_up_to_date)
	{
		...
		callOptimized(calcSE3Buffers,(reference, frame, referenceToFrame, finalLevel));
	    finalResidual = callOptimized(calcSE3WeightsAndResidual,(referenceToFrame));
	    callOptimized(calcSE3LGS,(ls7_kf));
	}

	lastSE3Hessian = ls7_kf.A;
...
#if defined(ENABLE_SSE)
void SE3KFTracker::calcSE3BuffersSSE(
...
#if defined(ENABLE_NEON)
void SE3KFTracker::calcSE3BuffersNEON(
		const TrackingReference* reference,
		Frame* frame,
		const SE3& referenceToFrame,
		int level, bool plotWeights)
...
void SE3KFTracker::calcSE3Buffers(
		const TrackingReference* reference,
		Frame* frame,
		const SE3& referenceToFrame,
		int level, bool plotWeights)
{
	...
	Eigen::Matrix3f rotMat = referenceToFrame.rotationMatrix().cast<float>();
...
#if defined(ENABLE_SSE)
SE3ResidualStruct SE3KFTracker::calcSE3WeightsAndResidualSSE(
		const SE3& referenceToFrame)
{
	...
	SE3ResidualStruct sumRes;
	memset(&sumRes, 0, sizeof(SE3ResidualStruct));
...
#if defined(ENABLE_NEON)
SE3ResidualStruct SE3KFTracker::calcSE3WeightsAndResidualNEON(
		const SE3& referenceToFrame)
{
	return calcSE3WeightsAndResidual(
			referenceToFrame);
}
#endif


SE3ResidualStruct SE3KFTracker::calcSE3WeightsAndResidual(
		const SE3& referenceToFrame)
{
...
#if defined(ENABLE_SSE)
void SE3KFTracker::calcSE3LGSSSE(LGS7_KF &ls7_kf)
{
	LGS4 ls4;
	LGS6 ls6;
...
#if defined(ENABLE_NEON)
void SE3KFTracker::calcSE3LGSNEON(LGS7_KF &ls7_kf)
{
	calcSE3LGS(ls7_kf);
}
#endif
...
void SE3KFTracker::calcSE3LGS(LGS7_KF &ls7_kf)
{
	LGS4 ls4;
	LGS6 ls6;
	ls6.initialize(width*height);
...
void SE3KFTracker::calcResidualAndBuffers_debugStart()
{
...
void SE3KFTracker::calcResidualAndBuffers_debugFinish(int w)
{
